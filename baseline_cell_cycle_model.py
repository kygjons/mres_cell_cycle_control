...
#Defining the function for baseline model:
def cell_cycle(y, t):
    # Tracked Species
...
#Defining each of the model parameters
k_deg = 1 #in min^-1
k_a =  50 #in min^-1
k_d =  0.05 #in min^-1
...
#Defining time interval for integration
...
#Defining each protein species array
...
#Calculationg total concentration of proteins in and out of complex
...
#Scaling the protein concentrations into percentage of maximum
...
#Plotting values from integration
...
cell_cycle_toy.set_xlim(0, 250)

#Adding shading for cell cycle phases
cell_cycle_toy.axvspan(t_interval[2250], t_interval[3050], facecolor="lightcoral")
cell_cycle_toy.axvspan(t_interval[3050], t_interval[3700], facecolor="lightgreen")
cell_cycle_toy.axvspan(t_interval[3700], t_interval[4050], facecolor="aquamarine")

#Title
cell_cycle_toy.set_title("Cell Cycle Duration = " + str((t_interval[4050]-t_interval[2250]).round()) + " minutes")
