...
#Function for Cln3 Screening Model
def cell_cycle(y, t):
    # Tracked Species
    Clb12, Clb56, Sic1, Clb12_Sic1, Clb56_Sic1, Clb34, Clb34_Sic1, Cln2, Cln3 = y
    ...
    dCln2dt = k_exp_Cln2*(((Cln2)/(1+(Cln2)))+((1)/(1+(Clb12)))+((1)/(1+(Clb56)))+((Cln3)/(1+(Cln3)))) - k_dil_Cln2*Cln2 - k_deg_Cln*Cln2*((Cln2)/(1+Cln2)+(Clb12)/(1+Clb12)+(Clb34)/(1+Clb34)+(Clb56)/(1+Clb56))
    
    dCln3dt = k_induction_Cln3 + k_exp_Cln3 - k_dil_Cln3*Cln3 - k_deg_Cln*Cln3*((Cln2)/(1+Cln2)+(Clb12)/(1+Clb12)+(Clb34)/(1+Clb34)+(Clb56)/(1+Clb56))
    
    solutions = [dClb12dt, dClb56dt, dSic1dt, d_Clb12_Sic1dt, d_Clb56_Sic1dt, dClb34dt, d_Clb34_Sic1dt, dCln2dt, dCln3dt]
    
    return solutions

#Parameters for Cln3 Screening Model
k_deg = 1 #in min^-1
k_a =  50 #in min^-1
k_d =  0.05 #in min^-1
...
Cln3_sol = sol.transpose()[8]

#Calculating total proteins in and out of complex for each protein species
...
Cln3_scaled = (Cln3_sol/np.amax(Cln3_sol))*100
